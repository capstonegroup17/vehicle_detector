...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
        self.conv = ACBlock(c1, c2, k, s, autopad(k, p), groups=g)  #Asymmetric convolution
...
class SELayer(nn.Module):
    def __init__(self, channel, reduction=1):
        ...
        self.fc2 = nn.Sequential(
            nn.Conv2d(channel, channel // reduction, 1, bias=False), #ACBlock(channel, channel // reduction, 1),   #nn.Conv2d(channel, channel // reduction, 1, bias=False),原始
            nn.ReLU(inplace=True),          #nn.ReLU(inplace=True),原始
            nn.Conv2d(channel, channel // reduction, 1, bias=False), #ACBlock(channel, channel // reduction, 1),   #nn.Conv2d(channel, channel // reduction, 1, bias=False),
            nn.Sigmoid()
        )
